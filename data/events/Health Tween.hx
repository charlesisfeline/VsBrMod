...
function postUpdate()
    if (healthTweenin) health = healthTweenObjThingy.x;
...
function onEvent(event)
{
    switch (event.event.name)
    {
        case 'Health Tween':
            ...
            FlxTween.tween(healthTweenObjThingy, {x: event.event.params[0]}, event.event.params[1],
                {ease: Reflect.field(FlxEase, flxease), onComplete: () -> healthTweenin = false});
    }
}
