...
function create()
{
	for (i in 0...2)
	{
		...
		if (i == 1)
			cinematicBar2 = cinematicBar;
		else
			cinematicBar1 = cinematicBar;
	}
	cinematicBar1.y = -10;
}
...
function onEvent(eventEvent)
{
	...
	if (eventEvent.event.name == "Cinematic Bars")
	{
		...
		else
		{
			...
			for (bar in [cinematicBar1, cinematicBar2])
			{
				var tween:FlxTween = FlxTween.tween(bar.scale, {y: ((FlxG.height + 20) / 2) * params[1]}, ((Conductor.crochet / 4) / 1000) * params[2],
					{ease: Reflect.field(FlxEase, flxease)});
				if (bar == cinematicBar1)
					cinematicBarTween1 = tween;
				else
					cinematicBarTween2 = tween;
			}
		}
	}
}
