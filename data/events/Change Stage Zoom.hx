...
function onEvent(eventEvent)
{
	...
	if (eventEvent.event.name == "Change Stage Zoom")
	{
		if (params[4])
		{
			...
			if (params[0])
			{
				if (defaultZoomTween != null)
					defaultZoomTween.cancel();
				defaultZoomTween = FlxTween.num(defaultCamZoom, params[5], ((Conductor.crochet / 4) / 1000) * params[6],
					{ease: Reflect.field(FlxEase, flxease)}, (val:Float) ->
					{
						defaultCamZoom = val;
					});
			}
			if (params[1])
			{
				if (bfZoomTween != null)
					bfZoomTween.cancel();
				bfZoomTween = FlxTween.num(strumLineBfZoom, params[5], ((Conductor.crochet / 4) / 1000) * params[6], {ease: Reflect.field(FlxEase, flxease)},
					(val:Float) ->
					{
						strumLineBfZoom = val;
					});
			...
			if (params[3])
			{
				if (gfZoomTween != null)
					gfZoomTween.cancel();
				gfZoomTween = FlxTween.num(strumLineGfZoom, params[5], ((Conductor.crochet / 4) / 1000) * params[6], {ease: Reflect.field(FlxEase, flxease)},
					(val:Float) ->
					{
						strumLineGfZoom = val;
					});
		...
		{
			if (params[0])
				defaultCamZoom = params[5];
			if (params[1])
				strumLineBfZoom = params[5];
			if (params[2])
				strumLineDadZoom = params[5];
			if (params[3])
				strumLineGfZoom = params[5];
		}
	}
}
