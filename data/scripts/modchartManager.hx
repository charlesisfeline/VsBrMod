...
function postUpdate(elapsed)
{
    ...
    for (p in strumLines)
    {
        p.forEach(function(strum)
        {
            ...
            if (strum.frame != null) strum.shader.frameUV = [strum.frame.uv.x, strum.frame.uv.y, strum.frame.uv.width, strum.frame.uv.height];
        ...
        p.notes.forEach(function(n)
        {
            ...
            if (n.isSustainNote && n.nextNote != null && n.nextNote.isSustainNote) nextCurPos = Conductor.songPosition - n.nextNote.strumTime;
            
            // sustain ends
            if (n.isSustainNote && n.nextSustain == null) nextCurPos = Conductor.songPosition - (n.strumTime + (Conductor.stepCrochet * 0.5));
            
            // clip to strum
            if (n.isSustainNote && n.wasGoodHit && curPos >= 0) curPos = 0;
            
            // calculate screen position for rotation and scaling inside shader
            var point = FlxPoint.weak();
            n.getScreenPosition(point, camHUD);
            n.shader.screenX = (n.origin.x + point.x - n.offset.x) + n.__strum.x;
            if (downscroll) n.shader.screenY = (n.origin.y + point.y - n.offset.y) - n.__strum.y;
            else
            ...
            for (mod in modTable[p][i]) // loop through each mod
            {
                if (mod[MOD_TYPE] == MOD_TYPE_NOTE)
                {
                    ...
                    if (mod[MOD_AUTODISABLE])
                    {
                        ...
                        if (!StringTools.contains(defaultValue, ".")) defaultValue += ".0"; // make sure it has a decimal so the shader knows its a float
...
////Modifier Functions/////
public function createModifier(name:String, value:Float, func:Dynamic, strumLineID:Int = -1, strumID = -1, defaultValue:Float = 0.0, autoDisable = true,
        modType:Int = 0)
{
    if (defaultValue == null) defaultValue = 0.0;
    if (autoDisable == null) autoDisable = true;
    if (strumLineID == null) strumLineID = -1;
    if (strumID == null) strumID = -1;
    if (modType == null) modType = MOD_TYPE_NOTE;
...
public function tweenModifierValue(name:String, newValue:Float, time:Float, easeFunc:Float -> Float)
{
    var mod = null;
    for (m in modifiers)
        if (m[MOD_NAME] == name) mod = m;
        
    if (mod == null) return; // cant find
    
    var startValue = mod[MOD_VALUE];
    FlxTween.num(startValue, newValue, time, {
