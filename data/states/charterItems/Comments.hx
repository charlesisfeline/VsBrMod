...
function update_moveWindow(group:FlxTypedSpriteGroup, ?forceUpdate:Bool = false, ?centerToMouse:Bool = false) {
    ...
    var titleText = test.filter(function(spr:FlxSprite) return spr.ID == 2).pop();
    test = group.members.copy();
...
function createCommentUI(xPos:Float, step:Float, ?title:String, ?text:String) {
    ...
    windowArea.ID = 1;
    windowArea.antialiasing = Options.antialiasing;
    ...
    commentTitleText.antialiasing = Options.antialiasing;
    ...
    commentText.ID = 3;
    commentText.antialiasing = Options.antialiasing;
...
function screenCenterUI(uiSprite) {
    uiSprite.setPosition((FlxG.width - uiSprite.bWidth) * 0.5, (FlxG.height - uiSprite.bHeight) * 0.5);
}
...
function loadComments() {
    ...
    for (data in coolio)
        createCommentUI(data.x, data.step, data.title, data.text);
}
