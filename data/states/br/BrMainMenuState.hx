...
function create()
{
	...
	menuItems.forEach((item:FlxSprite) ->
	{
		...
		if (item.ID == curSelected)
			FlxTween.tween(item, {x: 600 - item.width + 15}, (Conductor.stepCrochet / 1000) * 4, {ease: FlxEase.expoOut});
		else
...
function changeItem(change:Int = 0)
{
	...
	menuItems.forEach((item:FlxSprite) ->
	{
		...
		if (item.ID == curSelected)
			FlxTween.tween(item, {x: 600 - item.width + 15}, (Conductor.stepCrochet / 1000), {ease: FlxEase.quadInOut});
		else
